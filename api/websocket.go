...
import (
	...
	pylonRoom "github.com/y00rb/pylon-signaler/room"
...
func sendSdp(s *session, raw []byte) error {
	...
	dstConn, ok := pylonRoom.GetSession(s.ApiKey, s.Room, message.Args.Dst)
	if ok == false {
...
func HandleRootWSUpgrade(w http.ResponseWriter, r *http.Request) {
	...
	defer func() {
		if err := pylonRoom.DestroySession(apiKey, room, sessionKey); err != nil {
			log.Error().Err(err).
				Str("ApiKeyID", apiKey).
				Str("Room", room).
	...
	pylonRoom.StoreSession(apiKey, room, sessionKey, session)
